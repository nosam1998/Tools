...
def write_funded_loans_to_csv(data: [LoanFunded], filename):
    with open(filename, "w", newline="") as file:
        ...
        writer.writerow(["Borrower", "Loan Reason", "Payback Date", "Loan Principal", "SoLo Donation", "Lender Tip", "Payback Amount", "Status"])
        for loan in data:
            writer.writerow([
                loan.borrower,
                loan.loan_reason,
...
class GmailSearcher:
    ...
    def get_email_body(self, email_message):
        ...
        if email_message.is_multipart():
            ...
            if payload:
                body = payload.decode(email_message.get_content_charset("utf-8"), errors="replace")
