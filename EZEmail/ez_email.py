...
class EZEmail:
    def __init__(
            self, host: str, port: int | None, username: str, password: str
    ) -> None:
        """
    ...
    def get_emails_on_specific_date(self, dt: datetime):
        return self.imbox.messages(date__on=dt)

    def get_emails_before_date(self, dt: datetime):
        return self.imbox.messages(date__lt=dt)
    ...
    def emails_to_list(self, data):
        return [(uid, message) for uid, message in data]
